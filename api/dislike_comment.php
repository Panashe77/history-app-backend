...
try {
    ...
    if ($existing) {
        if ($existing['type'] === 'dislike') {
            ...
            $pdo->prepare("UPDATE comments SET dislikes = GREATEST(0, dislikes - 1) WHERE id = ?")->execute([$comment_id]);
            ob_clean(); echo json_encode(['success' => true, 'action' => 'removed_dislike']);
        } else {
            $pdo->prepare("UPDATE comment_likes SET type = 'dislike' WHERE comment_id = ? AND user_id = ?")->execute([$comment_id, $user_id]);
            $pdo->prepare("UPDATE comments SET dislikes = dislikes + 1, likes = GREATEST(0, likes - 1) WHERE id = ?")->execute([$comment_id]);
            ob_clean(); echo json_encode(['success' => true, 'action' => 'changed_to_dislike']);
        }
    } else {
        $pdo->prepare("INSERT INTO comment_likes (comment_id, user_id, type) VALUES (?, ?, 'dislike')")->execute([$comment_id, $user_id]);
    ...
    error_log("dislike_comment error: " . $e->getMessage());
    ob_clean(); http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'dislike_comment failed: ' . $e->getMessage()]);
}
?>
