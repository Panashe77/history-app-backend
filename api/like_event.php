    ...
    if ($event_id > 0 && $user_id > 0) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($existing_type == 'like') {
                ...
                $update_like_sql = "UPDATE event_likes SET like_type = 'like' WHERE event_id = ? AND user_id = ?";
                $update_like_stmt = $conn->prepare($update_like_sql);
                $update_like_stmt->bind_param("ii", $event_id, $user_id);
                $update_like_stmt->execute();
                
                $update_sql = "UPDATE events SET likes = likes + 1, dislikes = GREATEST(0, dislikes - 1) WHERE id = ?";
                $update_stmt = $conn->prepare($update_sql);
                $update_stmt->bind_param("i", $event_id);
                $update_stmt->execute();
            ...
            $insert_sql = "INSERT INTO event_likes (event_id, user_id, like_type) VALUES (?, ?, 'like')";
            $insert_stmt = $conn->prepare($insert_sql);
            $insert_stmt->bind_param("ii", $event_id, $user_id);
            $insert_stmt->execute();
            
            $update_sql = "UPDATE events SET likes = likes + 1 WHERE id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("i", $event_id);
            $update_stmt->execute();
